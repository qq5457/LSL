...
// 属性克制判断
const advantage = getElementAdvantage(playerCreature.element, enemyCreature.element);
const elementMult = advantage === 1 ? Lingshou.ELEMENT_DAMAGE_BONUS : (advantage === -1 ? Lingshou.ELEMENT_DAMAGE_REDUCTION : 1.0);
function getElementAdvantage(pElem, eElem) {
if (pElem === 'none' || eElem === 'none') return 0;
if (Lingshou.ELEMENT_ADVANTAGE[pElem] === eElem) return 1;
if (Lingshou.ELEMENT_ADVANTAGE[eElem] === pElem) return -1;
return 0;
}
...
if (side === 'p') raw = Math.round(raw * elementMult);
else raw = Math.round(raw * (elementMult >= 1 ? 1.0/elementMult : elementMult));
side === 'p' ? (ehp = Math.max(0, ehp - raw)) : (php = Math.max(0, php - raw));
if (wantLog) logs.push({ t: 'atk', txt: `${side === 'p' ? lib.displayName(playerCreature) : enemyCreature.name}发动【${tag}】，造成${raw}点伤害${elementMult !== 1.0 ? (elementMult > 1 ? '(克制)' : '(被克制)') : ''}` });
}
let round = 0;
...
let tier = 'normal';
if (map.allowBoss && lib.canSpawnBoss(map.id) && Math.random() < 0.018) tier = 'boss';
else if (map.allowElite && Math.random() < 0.13) tier = 'elite';
const isBaby = lib.canSpawnBaby(map.id) ? lib.rollBaby() : false;
...
const qualities = ['粗制','精良','传家'];
const q = qualities[Math.random() < 0.65 ? 0 : (Math.random() < 0.7 ? 1 : 2)];
const slotNameMap = { weapon: '兵刃', armor: '护甲', accessory: '饰品' };
const nameMap = {
weapon: { '粗制':['朽木剑','青铜刀','铁枪'], '精良':['精良朽木剑','精良青铜刀','精良铁枪'], '传家':['传家朽木剑','传家青铜刀','传家铁枪'] },
armor: { '粗制':['布甲','皮甲','铁甲'], '精良':['精良布甲','精良皮甲','精良铁甲'], '传家':['传家布甲','传家皮甲','传家铁甲'] },
accessory: { '粗制':['铜戒','玉坠','兽骨链'], '精良':['精良铜戒','精良玉坠','精良兽骨链'], '传家':['传家铜戒','传家玉坠','传家兽骨链'] }
};
const name = lib.choice(nameMap[slot][q]);
const cfg = Lingshou.EQUIP_QUALITY_PCT[q];
const pct = { atk:0, def:0, hp:0, spd:0 };
const bonuses = Lingshou.EQUIP_SLOT_BONUS[slot] || [];
bonuses.forEach(stat => {
pct[stat] = parseFloat(lib.rand(cfg.mainMin, cfg.mainMax).toFixed(3));
});
// 额外词条
const extra = [];
for (let i=0; i<cfg.extraCount; i++) {
const estats = Lingshou.EQUIP_EXTRA_STATS.slice();
const chosen = lib.choice(estats);
extra.push({ [chosen]: parseFloat(lib.rand(Lingshou.EQUIP_EXTRA_RANGE.min, Lingshou.EQUIP_EXTRA_RANGE.max).toFixed(3)) });
}
return { id: lib.uid(), slot, slotName: slotNameMap[slot], name, quality: q, pct, extra, itemLevel: map.minLv };
};
lib.equipScore = function(eq) {
if (!eq || !eq.pct) return 0;
let score = (eq.pct.atk||0)*1.3 + (eq.pct.def||0)*1.2 + (eq.pct.hp||0)*0.15 + (eq.pct.spd||0)*1.5;
(eq.extra||[]).forEach(ext => {
score += (ext.atk||0)*1.3 + (ext.def||0)*1.2 + (ext.hp||0)*0.15 + (ext.spd||0)*1.5;
});
return score;
};
// ========== 单次遭遇结算 ==========
...
if (wantLog) {
const elementStr = enemy.element !== 'none' ? `·${enemy.element}属性` : '';
logs.push({ t: 'title', txt: `遭遇：${enemy.isBaby ? enemy.name+'「宝」' : enemy.name}（${Lingshou.TIER_LABEL[enemy.tier]} Lv.${enemy.level}${elementStr}）` });
}
const result = { caught:null, gold:0, exp:0, petExpGain:0, win:battle.win, soulGain:0, matGain:0, refinedAuto:false };
if (battle.win) {
...
if (Math.random()<0.10) { const m=lib.randInt(1,3); state.player.mat+=m; result.matGain=m; if(wantLog) logs.push({t:'get',txt:`拾取：装备材料+${m}`}); }
if (enemy.tier!=='normal' && Math.random()<0.12) { const s=lib.randInt(1,3); state.player.soul+=s; result.soulGain=s; if(wantLog) logs.push({t:'get',txt:`拾取：兽魂+${s}`}); }
if (Math.random()<0.02) { const eq = lib.generateEquipment(map); state.equipInventory.push(eq); if(wantLog) logs.push({t:'get',txt:`拾取装备：${eq.name}`}); }
...
const soul = lib.soulValue(pet); state.player.soul+=soul; result.refinedAuto=true; result.soulGain+=soul;
if(wantLog) logs.push({t:'get',txt:`发现：${lib.displayName(pet)}（自动炼化兽魂+${soul}）`});
} else { state.pets.push(pet); result.caught=pet; state.stats.totalCatches++; lib.recordCatch(map.id, pet.tier); if(wantLog) logs.push({t:'get',txt:`发现：${lib.displayName(pet)}，已收入仓库！`}); }
}
} else {
if (wantLog) { battle.logs.forEach(l => logs.push(l)); logs.push({ t:'lose', txt:'战斗失利，主力负伤撤退。' }); }
const gold = Math.round(enemy.level*0.5); const exp = Math.round(enemy.level*0.6)+1;
state.player.gold += gold; grantPlayerExp(exp); result.gold=gold; result.exp=exp;
}
...
if (r.caught) { const key = lib.displayName(r.caught); harvest.caught[key] = (harvest.caught[key]||0)+1; }
}
if (lib.showHarvestModal) lib.showHarvestModal(harvest, elapsedSec);
};
lib.ensureActivePet = function() {
if (!lib.activePet() && state.pets.length) state.activePetId = state.pets[0].id;
